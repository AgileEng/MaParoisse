Ext.define('MaParoisse.view.parametrages.ConseilController', {
	...
	/*
	 * use currentCouncil to store the current council record here
	 * when loading data from server set the data here
	 * and update the view from this data
	 * 
	 * when applying changes sync this object with the UI representation
	 * and then send the data from currentCouncil to the server
	 */
	currentCouncil: null,
	...
	refreshGridView: function(){
		this.getView().getComponent('bureauGrid').getView().refresh();
	},
	...
	onBeforeGridCellEdit: function(editor, context, eOpts){
		if(context.field === 'positionName'/* && ((context.record.get('positionId') != 10) || (context.record.get('typeId') != 10))*/){
			return false;
		} else if((context.record.get('typeId') === 20 || context.record.get('typeId') === 30 || context.record.get('typeId') === 40) && (context.field === 'entryDate' || context.field === 'firstElectionDate' || context.field === 'nextRenewalDate')){
			return false;
		} else {
			return true;
		}
	...
	onAddClicked: function(btn, eOpts){
		...
		var cellEditingPlugin = grid.getPlugin('memberEditingPlugin');
		
		//Initialize empty/default Council Member
		var newRec = Ext.create('MaParoisse.model.CouncilMember', {
	...
	onSaveClicked: function(){
		...
		this.updateCurrentCouncil(function(){
			var council = vc.currentCouncil.getDataObjectExt();
			council.endDate = Ext.Date.format(council.endDate, 'd-m-Y');
			if(typeof council.startDate != 'string'){
				council.startDate = Ext.Date.format(council.startDate, 'd-m-Y');
			}
			...
			var req = Ext.create('MaParoisse.lib.JsonRPC', {
				...
				listeners: {
					success: function () {
						...
						vc.releaseResources();
						vc.setCurrentCouncil(arguments[0].BODY.council);
					},
					error: function () {}
				}
			});
	...
	onDeleteClicked: function(){
		var me = this,
			memStore = this.getView().getComponent('membersGrid').getStore(),
			bureauStore = this.getView().getComponent('bureauGrid').getStore(),
			membersSelModel = this.getView().getComponent('membersGrid').getSelectionModel(),
			bureauSelModel = this.getView().getComponent('bureauGrid').getSelectionModel();
		
		if(membersSelModel.hasSelection()){
			me.doDeleteMember(membersSelModel.getSelection()[0], memStore);
		} else if(bureauSelModel.hasSelection()){
			me.doDeleteMember(bureauSelModel.getSelection()[0], bureauStore);
		}
	},
	...
	onElectionsClicked: function(){
		...
		vc.getView().isSafeToClose(function(){
			var councilId = vc.currentCouncil.get('id');
	...
	onBureauGridSelectionChange: function(selModel, record, index, eOpts){
		var membersSelModel = this.getView().getComponent('membersGrid').getSelectionModel();
		if(membersSelModel.hasSelection()){
			membersSelModel.deselectAll();
		}
	},
	
	onMembersGridSelectionChange: function(selModel, selected, eOpts){
		var bureauSelModel = this.getView().getComponent('bureauGrid').getSelectionModel();
		if(bureauSelModel.hasSelection()){
			bureauSelModel.deselectAll();
		}
	},
	
	onGridItemContextMenu: function(view, rec, item, index, e, eOpts) {
		var me = this,
			memStore = this.getView().getComponent('membersGrid').getStore(),
			bureauStore = this.getView().getComponent('bureauGrid').getStore();
		...
		if(rec.get('typeId') == 10 && rec.get('positionId') == 10){
			menu.add({
				icon: null,
		        ...
		        handler: function(){
		        	me.doSetMemberAsOfficio(rec, 30);
		        }
			}, {
				icon: null,
				glyph: 'xe063@iconFont',
		        ...
		        handler: function(){
		        	me.doSetMemberAsOfficio(rec, 40);
		        }
			}, '-', {
				icon: null,
		...
		} else if(rec.get('typeId') != 10 && rec.get('positionId') == 10){
			menu.add({
				icon: null,
				glyph: 'xe03d@iconFont',
		        iconAlign: 'left',
		...
		} else if(rec.get('positionId') != 10 || rec.get('typeId') != 10){
			menu.add({
				icon: null,
				glyph: 'xe063@iconFont',
		        iconAlign: 'left',
	...
	updateCurrentCouncil: function(callback){
		...
		if(this.isValidCouncil()){
			...
			this.currentCouncil.set('endDate', endDateField.getValue());
			this.currentCouncil.set('members', this.getCouncilMembersData());
			callback();
		} else {
	...
	*/
	isValidCouncil: function(){
		var valid = true,
			view = this.getView(),
			membersGrid = view.getComponent('membersGrid'),
			ordinaryMembersStore = membersGrid.getStore(),
			bureauMembersGrid = view.getComponent('bureauGrid'),
			bureauMembersStore = bureauMembersGrid.getStore(),
			endDateField = view.getComponent('fieldContainer').getComponent('endDateField');
		
		var bureauGridView = bureauMembersGrid.getView(),
			bureauNodes = bureauGridView.getNodes();
		
		for(var i = 0; i < bureauNodes.length; i++){
			var rec = bureauGridView.getRecord(bureauNodes[i]);
			bureauMembersGrid.validateRow(bureauMembersGrid.getColumnIndexes(bureauMembersGrid), rec, bureauMembersStore.indexOf(rec), true);
		}
		
		var membersGridView = membersGrid.getView(),
			membersNodes = membersGridView.getNodes();
		
		for(var i = 0; i < membersNodes.length; i++){
			var rec = membersGridView.getRecord(membersNodes[i]);
			membersGrid.validateRow(membersGrid.getColumnIndexes(membersGrid), rec, ordinaryMembersStore.indexOf(rec), true);
		}
		
		ordinaryMembersStore.each(function(m){
			var cureCount = 0,
				maireCount = 0,
				maireAnnexeCount = 0;
			...
			/*
			 * validate on business logic level
			 * for example: president <=1
			 * 				tresorier <=1
			 * 				maire, cure etc. <= 1
			 */
			switch(m.get('typeId')){
			...
			if(maireAnnexeCount > 1 || maireCount > 1 || cureCount > 1){
				valid = false;
				return valid;
			}
		});
		
		bureauMembersStore.each(function(m){
			...
			if (presidentCount > 1 || secreatireCount > 1 || tresorierCount > 1){
				valid = false;
				return valid;
			}
		});
		
		if(!endDateField.isValid() || typeof endDateField.getValue() != 'object' || endDateField.getValue() == null){
			valid = false;
		}
	...
	getCouncilMembersData: function(){
		var me = this;
		var membersStore = this.getView().getComponent('membersGrid').getStore(),
			bureauStore = this.getView().getComponent('bureauGrid').getStore();
		
		var councilMembers = [];
		
		membersStore.each(function(rec){me.prepareMemberForServer(rec, councilMembers);});
		bureauStore.each(function(rec){me.prepareMemberForServer(rec, councilMembers);});
		
		Ext.Array.each(this.deletedMembers, function(rec){me.prepareMemberForServer(rec, councilMembers);});
	...
	prepareMemberForServer: function(record, membersArr){
		...
		var member = record.getDataObjectOpt(options={serialize: true});
		...
		if(member.dbState != 3 && !record.phantom){
			if(record.isModified('employeeAddress') || record.isModified('employeePostCode') || record.isModified('employeeTown')){
				member.employee.address.dbState = 2;
			}
			
			if(record.isModified('employeePhone') || record.isModified('employeeEmail')){
				member.employee.contact.dbState = 2;
			}
			
			if(record.isModified('employeeLastName') || record.isModified('employeeFirstName') || member.employee.contact.dbState === 2 || member.employee.address.dbState === 2){
	...
	releaseResources: function(){
		var membDs = this.getView().getComponent('membersGrid').getStore(),
			bureauDs = this.getView().getComponent('bureauGrid').getStore();
		
		membDs.removeAll();
		bureauDs.removeAll();
		this.currentCouncil = null;
		this.deletedMembers = [];
		this.getView().getComponent('fieldContainer').getComponent('endDateField').reset();
	},
	...
	recordValidationRenderer: function(value, metaData, record, rowIndex, colIndex, store, view){
	...
	doElectMember: function(member, position){
		if(this.positionIsAvailable(position)){
			var ordinaryMemberStore = this.getView().getComponent('membersGrid').getStore(),
				bureauMemberStore = this.getView().getComponent('bureauGrid').getStore();
	...
	doSetMemberAsOfficio: function(member, position){
		if(this.typeIsAvailable(position)){
			...
			//officio members do not have election date and entry date so set them empty
	...
	doSetMemberAsOrdinary: function(member){
		var me = this,
			view = me.getView(),
			membersGrid = view.getComponent('membersGrid'),
			bureauGrid = view.getComponent('bureauGrid'),
			ordinaryMemberStore = membersGrid.getStore(),
			bureauMemberStore = bureauGrid.getStore();
	...
	positionIsAvailable: function(position){
		var ds = this.getView().getComponent('bureauGrid').getStore(),
			available = true;
	...
	typeIsAvailable: function(type){
		var ds = this.getView().getComponent('membersGrid').getStore(),
		available = true;
		ds.each(function(rec){
		...
		Ext.create('MaParoisse.view.receipts.DocGenWindow', {
			...
			submitFunction: function(btn){
				...
				if(yearField.isValid()){
					window.open(
						'../CefraForm?number=council&ownerId=' + AccBureau.Context.principal.data.compId
	...
	onPrintBoardProtocolClicked: function(){
		Ext.create('MaParoisse.view.receipts.DocGenWindow', {
			...
			submitFunction: function(btn){
				...
				if(yearField.isValid()){
					window.open(
						'../CefraForm?number=councilProtocol&ownerId=' + AccBureau.Context.principal.data.compId
						+ '&year=' + yearField.getValue(),
						'_Print');
			    	view.close();
				}
