...
/**
 * Servlet implementation class SuppliersImportServlet
 */
public class SuppliersImportServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public SuppliersImportServlet() {
		super();
	}
				...
				if(items != null) {
					...
					while (iter.hasNext()) {  
						...
						if(!item.isFormField() && item.getSize() > 0) {  
							...
							File fileSuppliers = new File(fileRepository, remoteName);
							item.write(fileSuppliers);
							
							// import
							PartyService partyService = 
								(PartyService) ServiceLocator.getInstance().getService(Services.PARTY);
							AEResponse aeResponse = partyService.importSuppliers(
									fileSuppliers, 
									invContext);
							fileSuppliers.delete();
