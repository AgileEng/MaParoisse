...
public class ImageServlet extends HttpServlet {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 4556980506494932662L;
	...
	public void init() throws ServletException {
		...
		this.imagePath = AEApp.getInstance().getProperties().getProperty(AEApp.imageRepository);
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {
			...
			// Get requested image.
			String requestedImage = request.getParameter("name");

			// Check if file name is actually supplied to the request URI.
			if (AEStringUtil.isEmpty(requestedImage)) {
			...
			try {
				...
				AEApp.logger().error("Error flushing the response", ex);
			}
		}
	...
	private static void close(Closeable resource) {
		if (resource != null) {
			try {
				...
				AEApp.logger().error("Cannot close closeable resource", e);
			}
		}
	}
